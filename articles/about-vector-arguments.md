...
The `*apply_byname()` family of functions are used internally by many
functions in `matsbyname`. These functions
...
allow additional arguments, besides the matrices that are being
transformed, which are passed via `.FUNdots`. Getting `.FUNdots` right
can be challenging, and this vignette provides some background and
explanation to those challenges. After reading this vignette, both
callers of these functions and programmers who use these functions will
be in a better position to take advantage of the various
`*apply_byname()` functions.

## An example

To see how the `.FUNdots` argument works, let’s make an example function
that takes advantage of
[`unaryapply_byname()`](https://matthewheun.github.io/matsbyname/reference/unaryapply_byname.md).
`mysum()` adds entries in matrix `a`, the result depending upon the
value of `margin`.
...
Structuring `mysum()` as shown above provides several interesting
...
In the above examples, `margin` was only `1` or `2`, not `c(1, 2)` for
the list and data frame examples. Let’s see what happens when
...
To understand better what is happening, let’s try when the list argument
...
`margin = c(1, 2)` is interpreted by
[`unaryapply_byname()`](https://matthewheun.github.io/matsbyname/reference/unaryapply_byname.md)
as “use `margin = 1` for the first `m` in the list, and use `margin = 2`
for the second `m` in the list.

Now we see why passing `margin = c(1, 2)` failed for a list of length
`3` (`list(m, m, m)`):
[`unaryapply_byname()`](https://matthewheun.github.io/matsbyname/reference/unaryapply_byname.md)
applied `margin = 1` for the first `m`, `margin = 2` for the second `m`.
But the third `m` had no margin available for it.

We also see why passing `margin = c(1, 2)` was successful for a list of
length `2` (`list(m, m)`):
[`unaryapply_byname()`](https://matthewheun.github.io/matsbyname/reference/unaryapply_byname.md)
applied `margin = 1` for the first `m`, `margin = 2` for the second `m`.
In that case, we had exactly as many items in margin (2) as items in the
list passed to `a` (2).

Now that we understand the problem, how do we fix it? In other words,
how can we apply `margin = c(1, 2)` to all entries in `list(m, m, m)`?
Or, better yet, how could we apply `margin = 1` to the first `m`,
`margin = 2` to the second `m`, and `margin = c(1, 2)` to the third `m`?

## The fix

Fixes to the problem identified above can be provided by either the
caller or (partially) by the programmer in three ways:
...
`c(1, 2)` in a [`list()`](https://rdrr.io/r/base/list.html), the caller
indicates “Take this margin (`c(1, 2)`), replicate it as many times as
we have items in our `a` list, using one `c(1, 2)` for each item in the
list.”
...
The caller can also supply different `margin`s for each item in the list
of matrices.
...
But the caller must provide either `1` or `length(a)` items in the
...
automatically wrap vector arguments in lists. To that end, `matsbyname`
includes the
[`prep_vector_arg()`](https://matthewheun.github.io/matsbyname/reference/prep_vector_arg.md)
function.
[`prep_vector_arg()`](https://matthewheun.github.io/matsbyname/reference/prep_vector_arg.md)
uses heuristics to wrap vector arguments in lists, if needed and when
possible.

`mysum2()` demonstrates the use of
[`prep_vector_arg()`](https://matthewheun.github.io/matsbyname/reference/prep_vector_arg.md).
...
- the vector argument’s length is greater than 1 but not equal to the
  length of a,

then
[`prep_vector_arg()`](https://matthewheun.github.io/matsbyname/reference/prep_vector_arg.md)
wraps the vector argument (in this case `margin`) in a
[`list()`](https://rdrr.io/r/base/list.html), thereby relieving the
...
Note that if the length of the vector argument is equal to the length of
the list in `a`, the caller’s intention is ambiguous, and the vector
...
If the caller wants `c(1, 2)` to be applied to each item in the `a`
...
The reason
[`prep_vector_arg()`](https://matthewheun.github.io/matsbyname/reference/prep_vector_arg.md)
cannot *always* wrap vector arguments in a list is that data frame
...
It would be a mistake to wrap `DF2$margin` in a
[`list()`](https://rdrr.io/r/base/list.html) for the following call to
`mysum2`, because the caller’s intent is clearly “apply `margin = 1` for
...
The good news is that within the context of a data frame, the caller’s
...
Dealing with vector arguments to the various `*apply_byname()` functions
can be tricky. But there are three ways to solve any problems that
arise:
