---
title: "Writing Custom Functions for <tt>matsbyname</tt>"
...
`FUN` is mapped as expected over `a` (in the case of `unaryapply_byname` and `cumapply_byname`) 
...
The various `*apply_byname` functions allow users to extend the functionality of the `byname` 
package as needed for any problem domain.
The functions are used extensively in `byname` itself.
In fact, all `byname` functions utilize the `*apply_byname` functions,
