...
|--:|:------------------------------
`FUN` | a unary function in the case of `unaryapply_byname`, a binary function in the case of `binaryapply_byname` and `cumapply_byname`
...
`unaryapply_byname` applies `FUN` to a single matrix, a list of matrices, or a column in a data frame
that contains matrices.
The `rowcoltypes` argument must be one of 

"all", "transpose", "row", "col", "none"
