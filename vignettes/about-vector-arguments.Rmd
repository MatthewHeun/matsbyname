...
`unaryapply_byname()` applied `margin = 1` for the first `m`, `margin = 2` for the second `m`.
There we exactly as many items in margin (2) as items in `m` (2).
...
and use one `c(1, 2)` for each item in the list."
...
To the extent possible, the programmer should remove burdens on users of their functions.
So, it would be helpful if there were a way to automatically wrap in lists.
To that end, `matsbyname` includes `prep_vector_arg()`.
...
It would be a mistake to wrap `DF$margin` in a `list()` 
...
The good news is that within the context of a data frame, the caller's intent will always be clear.
...
But there exist three ways to solve any problems that arise:
