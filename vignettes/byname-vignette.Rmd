...
The bookkeeping is cumbersome when analyses span many years (for example),
requiring the analyst to know *a-priori* 
the set of all rows and columns present in **all** years. 
Indeed, in this example, tracking the emergence and disappearance of products and industries
over many years should be done by the data itself, not the analyst.
...
The bookkeeping is problematic when matrix inversion is required after 
...
* missing rows or columns are inserted automatically and only as needed and
* zero rows or columns can be removed as needed
...
The challenges described above can be solved
...
tidy^[See http://tidyr.tidyverse.org/articles/tidy-data.html.] data frames
...
* Usse <tt>matsindf::expand_to_tidy</tt> to create a tidy data frame of matrix elements.
* <tt>tidyr::spread</tt> the data as necessary.
* Graph or perform other manipulations of the data.

For additional details and an example of this workflow, 
see the <tt>matsindf</tt> vignette.
