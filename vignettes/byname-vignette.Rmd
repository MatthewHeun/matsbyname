...
ensure size conformity of operands,
...
that contains the quantity of each product consumed by industries in final demand.
...
Matrices may have different numbers or different names of rows or columns
if, for example, 
the products or industries changes across time periods. 
...
The solution (adding rows and columns of zeroes) 
...
Matrix functions provided by <tt>R</tt> do not ensure type conformity for matrix operands
to matrix algebra functions. 
For example, columns of one matrix must represent the same entity 
as the rows of another matrix for matrix multiplication. 
If the columns of **A** represent countries, 
...
The <tt>byname</tt> package performs smart matrix operations that

* respect row and column names in matrix operations 
    + by inserting rows and columns of zeroes as necessary
    + by re-ordering rows and columns so that operands conform, and
* optionally provide row and column types and enforce type conformity for matrix operations. 
...
the <tt>byname</tt> package contains many additional 
...
* <tt>invert_byname</tt>, and 
* <tt>transpose_byname</tt>.

The full list of functions can be found with <tt>?byname</tt>
and clicking the <tt>Index</tt> link.

Furthermore, <tt>byname</tt> works well with its sister package, <tt>matsindf</tt>.
<tt>matsindf</tt> creates data frames
whose entries are not numbers but entire matrices, thereby
enabling the use of <tt>byname</tt> functions in 
[<tt>tidyverse</tt>](http://www.tidyverse.org)
functional programming.

When used together, <tt>byname</tt> and <tt>matsindf</tt> 
...
performed <tt>byname</tt>.


## <tt>byname</tt> features

The <tt>byname</tt> package has several features
...
Data types are character strings stored
...
As careful tracking of units is a necessary but not sufficient condition to achieve
correctness of scalar calculations,
so careful tracking of row and column types is a necessary but not sufficient condition to achieve
correctness of matrix algebra calculations.
Because <tt>byname</tt> keeps track of row and column types automatically,
...
thereby taking advantage of <tt>byname</tt>'s
...
Another feature of the <tt>byname</tt> package
is that it works 
when lists of matrices are arguments,
...
[tidy](http://tidyr.tidyverse.org/articles/tidy-data.html) data frames
...
* <tt>byname</tt> works well with lists and 
* the [<tt>tidyr</tt>](http://tidyr.tidyverse.org) and 
[<tt>dplyr</tt>](http://dplyr.tidyverse.org) packages work with data frames,

<tt>byname</tt> functions can be used with 
[<tt>tidyr</tt>](http://tidyr.tidyverse.org) and 
[<tt>dplyr</tt>](http://dplyr.tidyverse.org) functions 
(such as
<tt>spread</tt> and <tt>mutate</tt>)
to perform matrix algebra within data frames
of matrices.
A single <tt>byname</tt> instruction performs the same operation
...
to allow use of <tt>byname</tt> functions with 
