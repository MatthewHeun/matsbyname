...
because a single list of operands is ambiguous.
...
The `.summarise = TRUE` argument works when there are matrices in a `matsindf` data frame.
...
`df_to_aggregation_map()` assists converting from a two-column data frame 
...
df
bands_membs_agg_map <- df_to_aggregation_map(df, few_colname = "band", many_colname = "member")
bands_membs_agg_map
df_to_aggregation_map(df, few_colname = "role", many_colname = "member")
```
...
```{r}
aggregation_map_to_df(bands_membs_agg_map, 
                      few_colname = "bands",
...
the functions
`df_to_aggregation_map()` and `aggregation_map_to_df()` 
