...
test_that("constant_vector_byname() function works", {
  ...
  expect_equal(
    constant_vector_byname(single_mat, k = 3, col_name = "output_column"), 
  ...
  expect_equal(
    ...
    matrix(8, nrow = 2, ncol = 1, dimnames = list(c("r1", "r2"), "output_column"))
  )
  ...
  expect_equal(res$constant_vec[[2]][["p2", "Product"]], 54)
  expect_equal(res$constant_vec[[3]][["p3", "Product"]], 54)
  
  # Checking sums
  res2 <- res %>%
    dplyr::mutate(
      sum_constant = matsbyname::sumall_byname(constant_vec)
    )
  # Check
  expect_equal(res2$sum_constant[[1]], 108)
