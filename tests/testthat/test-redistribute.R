test_that("redistribute_byname() errors as expected", {
  expect_error(
    matsbyname::redistribute_byname(a = 42, margin = c(1, 2, 3)), 
    "margin must have length 1 or 2 in matsbyname::redistribute_byname\\(\\)"
  )
  expect_error(
    matsbyname::redistribute_byname(a = 42, margin = 4), 
    "margin must be 1, 2, or c\\(1, 2\\) in matsbyname::redistribute_byname\\(\\)"
  )
  expect_error(
    matsbyname::redistribute_byname(a = 42, margin = c(1, 1)), 
...
test_that("redistribute_byname() works with row redistribution", {
  ...
  res <- matsbyname::redistribute_byname(a, "r3", margin = 1)
  ...
  res2 <- matsbyname::redistribute_byname(a, c("r1", "r3"), margin = 1)
  ...
  # Test with a list
  res3 <- matsbyname::redistribute_byname(list(a, a), "r3", margin = 1)
...
test_that("redistribute_byname() works with column redistribution", {
  ...
  res <- matsbyname::redistribute_byname(a, "c3", margin = 2)
  ...
  res2 <- matsbyname::redistribute_byname(a, c("c1", "c2"), margin = 2)
  ...
  # Test with a list
  res3 <- matsbyname::redistribute_byname(list(a, a, a), "c3", margin = 2)
  expected3 <- list(expected, expected, expected)
  expect_equal(res3, expected3)
})


test_that("redistribute_byname() works with a 0 column", {
  # Try redistributing a 0 column.
  a <- matrix(c(1, 2, 3, 0,
                4, 5, 6, 0), 
  ...
  res <- matsbyname::redistribute_byname(a, "c4", margin = 2)
  ...
  a2 <- matrix(c(1, 0,
               ...
               dimnames = list(c("r1", "r2", "r3"), 
                               c("c1", "c2")))
  res2 <- matsbyname::redistribute_byname(a2, "r3", margin = 1) |> 
    expect_warning("The following cannot be redistributed due to all zero values in the receiving rows: c2")
  expected2 <- matrix(c(2, 0,
                        4, 0), 
                      nrow = 2, ncol = 2, byrow = TRUE, 
                      dimnames = list(c("r1", "r2"), 
                                      c("c1", "c2")))
